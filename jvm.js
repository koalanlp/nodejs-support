...
class JVM {
   ...
   */
  constructor(java) {
    _defineProperty(this, "java", void 0);

    if (typeof java === 'undefined' || java === null || JVM.INSTANCE) throw TypeError('java는 null일 수 없고, JVM.init()은 한 번만 실행되어야 합니다.');
    this.java = java;
    JVM.INSTANCE = this;
  }

  static init(java) {
    if (JVM.INSTANCE) return JVM.INSTANCE;else return new JVM(java);
  }
  /**
   * Java class 반환
  ...
  static classOf(...path) {
    ...
    if (!JVM._classes.hasOwnProperty(clsName)) JVM._classes[clsName] = JVM.INSTANCE.java.import(clsName);
  ...
  static listOf(array) {
    ...
    for (item of array) list.add(item);
  ...
  static char(ch) {
    if (ch !== null && typeof ch !== 'undefined') return JVM.INSTANCE.java.newChar(ch);else return null;
  }
  /**
   * Make Java Set
