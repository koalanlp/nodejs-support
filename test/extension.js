...
describe('ExtUtil Module', () => {
    before(async() => {
        await initialize({packages: {CORE: 'LATEST'}})
    });
    ...
    describe('Alphabet Reading', () => {
        ...
        it('can read/write alphabet', () => {
            _.range(50).forEach(() => {
                ...
                ExtUtil.alphaToHangul(originalStr).should.equal(koreanStr);
                ExtUtil.hangulToAlpha(koreanStr).should.equal(originalStr);
                ExtUtil.isAlphaPronounced(koreanStr).should.be.true();
            });

            ExtUtil.alphaToHangul("삼성 갤럭시 S9").should.equal("삼성 갤럭시 에스9");
            ExtUtil.hangulToAlpha("이마트").should.equal("E마트");
            ExtUtil.isAlphaPronounced('E마트').should.be.false();
        });
    });
    ...
    describe('Hanja Reading', () => {
        ...
        it('can read Hanja', () => {
            for(const [hanja, hangul] of sample){
                ExtUtil.hanjaToHangul(hanja).should.equal(hangul);
            }

            ExtUtil.hanjaToHangul("한 女人이 길을 건넜다", false).should.equal("한 녀인이 길을 건넜다");
            ExtUtil.hanjaToHangul("한 女人이 길을 건넜다", true).should.equal("한 여인이 길을 건넜다");
        });

        it('can recognize Hanja', () => {
            ExtUtil.isHanja('金').should.deepEqual([true]);
            ExtUtil.isHanja('㹤').should.deepEqual([true]);

            ExtUtil.isHanja('김').should.deepEqual([false]);
            ExtUtil.isHanja("⺀⺁⺂⻱⻲⻳").every((x) => x).should.be.true()
        });
    });

    describe('Hangul Processor', () => {
        it('identifies hangul character', () => {
            ExtUtil.isCompleteHangul('k').should.deepEqual([false]);
            ExtUtil.isCompleteHangul('車').should.deepEqual([false]);
            ExtUtil.isCompleteHangul('\u1161').should.deepEqual([false]);
            ExtUtil.isJungsungJamo('\u1161').should.deepEqual([true]);
            ...
            for (const [ch, a, h, ih, j1, j2, j3] of _.zip(mixedSentence, isCompleteHangul, isHangul, isIncompleteHangul, isChosungJamo, isJungsungJamo, isJongsungJamo)) {
                ...
                if ("SN '.:".includes(ch)){
                    ...
                    j2.should.be.false();
                    j3.should.be.false();
                    je.should.be.equal("인플엔핑심각법적안전장는즘갤럭".includes(ch));
                }
            }

            let fragments = mixedSentence.split(" ");
            for (const fragment of fragments){
                ExtUtil.isHangulEnding(fragment).should.equal(!["SNS", "'인플루엔서'", "심각...", "미비:", "갤럭시S"].includes(fragment));
                ExtUtil.isJongsungEnding(fragment).should.equal(["쇼핑", "법적", "ㄱ씨는", "요즘"].includes(fragment));
            }
        });
        ...
        it('dissembles hangul', () => {
            ...
            for(const [a, j1, j2, j3] of _.zip(completeHangul, chosung, jungsung, jongsung)){
                if(a){
                    ExtUtil.isChosungJamo(j1).every((x) => x).should.be.true();
                    ExtUtil.isJungsungJamo(j1).some((x) => x).should.be.false();
                    ExtUtil.isJongsungJamo(j1).some((x) => x).should.be.false();

                    ExtUtil.getChosung(j1)[0].should.equal(j1);
                    (typeof ExtUtil.getJungsung(j1)[0] === 'undefined').should.be.true();
                    (typeof ExtUtil.getJongsung(j1)[0] === 'undefined').should.be.true();

                    ExtUtil.isChosungJamo(j2).some((x) => x).should.be.false();
                    ExtUtil.isJungsungJamo(j2).every((x) => x).should.be.true();
                    ExtUtil.isJongsungJamo(j2).some((x) => x).should.be.false();

                    (typeof ExtUtil.getChosung(j2)[0] === 'undefined').should.be.true();
                    ExtUtil.getJungsung(j2)[0].should.equal(j2);
                    (typeof ExtUtil.getJongsung(j2)[0] === 'undefined').should.be.true();

                    if(typeof j3 !== 'undefined'){
                        ExtUtil.isChosungJamo(j3).some((x) => x).should.be.false();
                        ExtUtil.isJungsungJamo(j3).some((x) => x).should.be.false();
                        ExtUtil.isJongsungJamo(j3).every((x) => x).should.be.true();

                        (typeof ExtUtil.getChosung(j3)[0] === 'undefined').should.be.true();
                        (typeof ExtUtil.getJungsung(j3)[0] === 'undefined').should.be.true();
                        ExtUtil.getJongsung(j3)[0].should.equal(j3);
                    }
                }else{
                    (typeof j1 === 'undefined').should.be.true();
                    (typeof j2 === 'undefined').should.be.true();
                    (typeof j3 === 'undefined').should.be.true();
                }
            }
        });
        ...
        it('assembles hangul', () => {
            _.range(100).forEach(() => {
                ...
                triplestr.should.be.equal(str);
                ExtUtil.isJongsungEnding(str).should.be.equal(ExtUtil.isJongsungJamo(codestr).reverse()[0]);
                ExtUtil.isJongsungEnding(str).should.be.equal(typeof code.reverse()[0][2] !== 'undefined');
                code.reverse();

                ExtUtil.getChosung(str).should.deepEqual(code.map((x) => x[0]));
                ExtUtil.getJungsung(str).should.deepEqual(code.map((x) => x[1]));
                ExtUtil.getJongsung(str).should.deepEqual(code.map((x) => x[2]));

                ExtUtil.dissembleHangul(str).should.equal(codestr)
            });

            let sampleString = "SNS '인플루엔서' 쇼핑 피해 심각... 법적 안전장치 미비: ㄱ씨는 요즘 ㄴ SNS에서 갤럭시S \u1100\u1100 \u11A8\u11A8";
            ExtUtil.assembleHangul(ExtUtil.dissembleHangul(sampleString)).should.equal(sampleString);
        });
    });
        ...
        it('corrects surface form', () => {
            for(const splits of map){
                ...
                for(const [verb, isVerb, rest, result] of testcases){
                    ExtUtil.correctVerbApply(verb, isVerb, rest).should.equal(result);
                }
            }
        })
    })
