...
class SentenceSplitter extends Function {
  ...
  static async sentences(paragraph) {
    ...
    return _jvm.JVM.toJsArray(promiseResult, _data.Sentence.fromJava);
  }
  ...
  async tag(...text) {
    ...
    for (let paragraph of text) {
      ...
      if (Array.isArray(paragraph)) {
        ...
        result.push(..._jvm.JVM.toJsArray(promiseResult, _data.Sentence.fromJava));
      }
    }
  ...
  async tagSentence(...sentence) {
    ...
    for (let paragraph of text) {
      ...
      if (Array.isArray(paragraph)) {
        ...
        result.push(_data.Sentence.fromJava(promiseResult));
      }
    }
  ...
  async analyze(...text) {
    ...
    for (let paragraph of text) {
      ...
      if (Array.isArray(paragraph)) {
        ...
        result.push(...promiseResult);
      } else if (paragraph instanceof _data.Sentence) {
...
function readDicEntry(entry) {
  return {
    ...
    'tag': _types.POS.withName(entry.getSecond().name_)
  };
}
/**
 * 사전 Wrapper
...
class Dictionary {
  ...
  async addUserDictionary(...pairs) {
    ...
    await this._api.addUserDictionaryPromise(_jvm.JVM.listOf(surfaceList), _jvm.JVM.listOf(tagList));
  }
  /**
   * 사전에 등재되어 있는지 확인합니다.
  ...
  async contains(word, ...posTags) {
    ...
    if (tags.length === 1) {
      let tag = tags[0];
      return await this._api.containsPromise(_jvm.JVM.pair(word, tag.reference));
    } else {
      let tagsRef = tags.map(tag => tag.reference);
      return await this._api.containsPromise(word, _jvm.JVM.setOf(tagsRef));
    }
  }
  /**
